/*
	Monte-Carlo Benchmark
		...
		Last updated: Dic 14, 2016

	This program uses a Monte-Carlo solution to solve the
...
{		
	...
	double rm=6467444.7;	//Earth center to Mesosphere 6467444.7 km in m
	...
	xa += vx;
	ya += vy;
	te = fmod ((double)(te + NM(random_eng())),(double)(2*pid));
	l = (-1.0/ltao)*log(1.0-random_eng());
	vx = l*cos(te);
	vy = l*sin(te);
	}while(((xa*xa)+(ya*ya)<(pow(rm,2))) && (ya>=rcali) && ((pow(xa,2)+pow(ya,2))>=(pow(rt,2))));	
...
{	
	...
	ti=clock();
	/**/
  	file=sprintf(FileName,"./MC_B_D_v1.0_%d_%d_%d.dat",name,SunPos,lambda);
	file++;
